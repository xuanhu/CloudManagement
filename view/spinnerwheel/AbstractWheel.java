...
package com.tg.cloudmanagement.view.spinnerwheel;
...
import com.tg.cloudmanagement.view.spinnerwheel.adapter.WheelViewAdapter;
...
public abstract class AbstractWheel extends View {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        ss.currentItem = this.getCurrentItemPosition();
    ...
    public ArrayList<SlideItemObj> getList(){
    	return mViewAdapter == null ? null: mViewAdapter.getList();
    }
    ...
    public void setViewAdapter(WheelViewAdapter viewAdapter) {
        ...
        int width = Tools.getDisplayMetrics(context).widthPixels/3;
        int height = getItemDimension() * mVisibleItems;
        recreateAssets(width, height);
        invalidateItemsLayout(true);
    }
    ...
    public CharSequence getCurrentItemString() {
    	if(mViewAdapter == null){
    		return "";
    	}
    	return mViewAdapter.getItemText(mCurrentItemIdx);
    }
    
    public SlideItemObj getCurrentItem(){
    	ArrayList<SlideItemObj> list = mViewAdapter.getList();
    	if(mCurrentItemIdx < 0 || mCurrentItemIdx > list.size() - 1){
    		return null;
    	}
    	if(mViewAdapter == null || list == null || list.size() == 0){
    		return null;
    	}
    	return list.get(mCurrentItemIdx);
    }
    ...
    public int getCurrentItemId(){
    	if(mViewAdapter == null || mViewAdapter.getItemsCount() <= 0){
    		...
    		return Integer.parseInt(mViewAdapter.getList().get(mCurrentItemIdx).id);
    	}
    }
    /**
    ...
    public void setCurrentItem(int index, boolean animated,boolean needNotify) {
        if (mViewAdapter == null || mViewAdapter.getItemsCount() == 0) {
            return; // throw?
        }
